...
export const UpdateAccount = ({ open, onOpenChange, onAccountCreated, accountId }: CreateAccountProps) => {
  ...
  const fetchAccount = async () => {
    ...
    try {
      ...
      // Formata o balance antes de preencher o form
      const formattedBalance = formatCurrency((accountData.balance * 100).toString());
      
      // Preenche o form com os dados da conta
      reset({
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px] bg-white rounded-lg">
        <form onSubmit={handleSubmit(onSubmit)}>
          <DialogHeader>
            <DialogTitle>Update {accountName}</DialogTitle>
            <DialogDescription>
